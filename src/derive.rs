...
impl Scheme {
        ...
        F: Fn(f64, f64, f64, f64) -> (f64, f64),
    {
        ...
        let mut scheme = Scheme(vec![0.0; num + 1], vec![0.0; num + 1]);
    ...
    * `f` - Multi step difference scheme
        ...
        F: Fn(&Vec<f64>, usize, f64, f64, f64, f64, DeriveFn) -> f64,
    {
        ...
        let (mut x_s, mut y_s) = (self.0[level - 1], self.1[level - 1]);
        ...
        while (t - 1.0).abs() > eps {
            loop {
                ...
                println!("x_next: {}, y_next: {}", x_next, y_next);
        ...
        F: Fn(&Vec<f64>, usize, f64, f64, f64, f64, DeriveFn) -> f64,
    {
        ...
        while (t - 1.0).abs() < eps {
            loop {
                ...
                if (x_next - x_s).abs() > eps && (y_next - y_s).abs() > eps {
                    break;
                } else {
                    (x_s, y_s) = (x_next, y_next);
                }
            ...
            (self.0[i], self.1[i]) = (x_next, y_next);
